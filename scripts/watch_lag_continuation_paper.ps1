...
$watchdogLog = Join-Path $repo ("logs\lag_continuation_paper_watchdog_" + $watchdogTs + ".log")
...
while ($true) {
    $cycle += 1
    $logFile = Join-Path $repo ("logs\lag_continuation_paper_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".jsonl")
    ...
    & $python "diagnostics_lag_continuation_paper_v1.py" `
