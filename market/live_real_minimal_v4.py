...
def monitor_live_real_minimal_v4(duration_seconds: int | None = None) -> None:
    ...
    if not cfg.enabled:
        ...
        return
    if cfg.shadow_only:
        print("[GUARD] live_real_minimal_v4 requires POLY_GUARDED_SHADOW_ONLY=false")
        return
    if not cfg.real_posts_enabled:
        print("[GUARD] live_real_minimal_v4 requires POLY_GUARDED_REAL_POSTS_ENABLED=true")
        return
    if cfg.allow_next_2:
        print("[GUARD] live_real_minimal_v4 requires POLY_GUARDED_ALLOW_NEXT_2=false for first live rollout")
        return
    if cfg.max_active_plans != 1:
        print("[GUARD] live_real_minimal_v4 requires POLY_GUARDED_MAX_ACTIVE_PLANS=1")
        return
    if cfg.min_shares_per_leg != 5:
        print("[GUARD] live_real_minimal_v4 expects POLY_GUARDED_MIN_SHARES=5 for first live rollout")
        return
    ...
    try:
        ...
        if restored_plan_ids and not startup_orders and startup_report.get("tracked_count", 0) == 0:
            print("[PERSIST_STALE] restored local state has no matching broker open orders; clearing local persistence")
