...
def reconcile_executor_with_broker_open_orders_v2(executor, broker_open_orders: List[Any]) -> Dict[str, Any]:
    ...
    for order in broker_open_orders or []:
        ...
        original_size = float(getattr(order, "original_size", 0.0) or 0.0)
        size_matched = float(getattr(order, "size_matched", 0.0) or 0.0)
