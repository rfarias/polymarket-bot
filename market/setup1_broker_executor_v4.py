...
class Setup1BrokerExecutorV4:
    ...
    def _mark_broker_leg_canceled(self, plan_id: str, leg: str, response: dict) -> None:
        payload = self.plan_broker_orders.get(plan_id, {}).get(leg)
        if not payload:
            return
        payload["cancel_response"] = response
    ...
    def evaluate_slot(self, *, slot_name: str, event_slug: str, signal: str, metrics: Optional[Dict[str, float]], secs_to_end: Optional[int], outcome_token_ids: Optional[Dict[str, str]] = None) -> List[str]:
        ...
        up_token_id = (outcome_token_ids or {}).get("UP") or f"{event_slug}:UP"
        down_token_id = (outcome_token_ids or {}).get("DOWN") or f"{event_slug}:DOWN"
        ...
        for leg, payload in broker_orders.items():
            if self.shadow_only:
                ...
                if order_id:
                    ...
                    self._mark_broker_leg_canceled(plan_id, leg, resp)
    ...
    def process_market_tick(self, *, slot_name: str, event_slug: str, signal: str, metrics: Optional[Dict[str, float]], secs_to_end: Optional[int], deadline_trigger: Optional[int] = None, outcome_token_ids: Optional[Dict[str, str]] = None) -> List[str]:
        logs = self.evaluate_slot(slot_name=slot_name, event_slug=event_slug, signal=signal, metrics=metrics, secs_to_end=secs_to_end, outcome_token_ids=outcome_token_ids)
