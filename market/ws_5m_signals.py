...
def _is_placeholder_pair(up: Dict[str, Any], down: Dict[str, Any]) -> bool:
    ...
    if None in (up_bid, up_ask, down_bid, down_ask):
        return True
    return (
        float(up_bid) <= PLACEHOLDER_BID_MAX
        and float(down_bid) <= PLACEHOLDER_BID_MAX
        and float(up_ask) >= PLACEHOLDER_ASK_MIN
        and float(down_ask) >= PLACEHOLDER_ASK_MIN
    )
...
async def monitor_5m_signals(duration_seconds: int = 20) -> None:
