...
def _decision_key(kind: str, reason: Optional[str], details: Optional[Dict[str, Any]]) -> str:
    return f"{kind}|{reason}|{details}"
...
async def simulate_5m_execution_v6(duration_seconds: int = 40) -> None:
    ...
    last_decision_key = {"next_1": None, "next_2": None}
                ...
                if time.time() - last_print >= 2:
                    ...
                    for slot_name in ("current", "next_1", "next_2"):
                        ...
                        if slot_name == "next_1" and new_state == STATE_EXIT_TRIGGERED and plan is None and not focus_announced:
                            ...
                            print("[DECISION] next_1: skipped_timing")
                            ...
                            last_decision_key[slot_name] = _decision_key("skipped_timing", f"secs_to_end={secs}", None)
                            ...
                            if slot_name == "next_1" and focus_slot != "next_2":
                                ...
                                focus_slot = "next_2"
                                focus_announced = True
                        elif slot_name in ("next_1", "next_2") and plan is None and new_state in (STATE_WATCHING, STATE_ARMED):
                            if not metrics:
                                decision = _decision_key("blocked", "metrics_unavailable", None)
                                if last_decision_key[slot_name] != decision:
                                    print(f"[DECISION] {slot_name}: blocked")
                                    print(f"[FILTER] {slot_name}: entry blocked -> metrics unavailable")
                                    last_decision_key[slot_name] = decision
                            else:
                                ...
                                if ok:
                                    ...
                                    if created_plan:
                                        for event in events:
                                            print(f"[PLAN] {slot_name}: {event}")
                                        print(f"[DECISION] {slot_name}: plan_created")
                                        ...
                                        last_decision_key[slot_name] = _decision_key("plan_created", None, details)
                                    else:
                                        decision = _decision_key("blocked", "duplicate_or_create_failed", details)
                                        if last_decision_key[slot_name] != decision:
                                            print(f"[DECISION] {slot_name}: blocked")
                                            for event in events:
                                                print(f"[FILTER] {slot_name}: {event}")
                                            print(f"[QUALITY] {slot_name}: {details}")
                                            last_decision_key[slot_name] = decision
                                else:
                                    decision = _decision_key("blocked", reason, details)
                                    if last_decision_key[slot_name] != decision:
                                        print(f"[DECISION] {slot_name}: blocked")
