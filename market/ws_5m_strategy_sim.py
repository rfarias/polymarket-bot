...
def _transition_reason(state: str, metrics: Optional[Dict[str, float]], secs_to_end: Optional[int], exit_trigger: Optional[int]) -> str:
    if state == STATE_EXIT_TRIGGERED:
        return f"secs_to_end <= {exit_trigger}"
    if not metrics:
        return "placeholder_or_incomplete"
    if state == STATE_ARMED:
        return f"sum_asks={metrics['sum_asks']} <= {ARBITRAGE_SUM_ASKS_MAX}"
    if state == STATE_WATCHING:
        return f"sum_asks={metrics['sum_asks']} <= {WATCH_SUM_ASKS_MAX}"
    return f"sum_asks={metrics['sum_asks']}"
...
async def simulate_5m_strategy(duration_seconds: int = 20) -> None:
    ...
    armed_snapshots: Dict[str, Optional[Dict[str, float]]] = {"next_1": None, "next_2": None}
            ...
            while time.time() - started_at < duration_seconds:
                ...
                if time.time() - last_print >= 2:
                    ...
                    for slot_name in ("current", "next_1", "next_2"):
                        ...
                        if new_state != states[slot_name]:
                            reason = _transition_reason(new_state, metrics, secs, exit_trigger)
                            print(f"[STATE] {slot_name}: {states[slot_name]} -> {new_state} | {reason}")
                            states[slot_name] = new_state
                            if slot_name in armed_snapshots and new_state == STATE_ARMED and metrics:
                                armed_snapshots[slot_name] = metrics
                        ...
                        if slot_name in armed_snapshots and states[slot_name] == STATE_ARMED and armed_snapshots[slot_name]:
                            arm = armed_snapshots[slot_name]
                            print(f"[SIM] armed_prices -> up_ask={arm['up_ask']} | down_ask={arm['down_ask']} | sum_asks={arm['sum_asks']}")
        finally:
            hb.cancel()
