...
def _transition_state(prev: str, metrics: Optional[Dict[str, float]], stable_count: int, secs_to_end: Optional[int], exit_trigger: Optional[int]) -> str:
    if prev in (STATE_SIM_PLANNED, STATE_SIM_EXPIRED):
        return prev
...
async def simulate_5m_strategy_v2(duration_seconds: int = 20) -> None:
            ...
            while time.time() - started_at < duration_seconds:
                ...
                if time.time() - last_print >= 2:
                    ...
                    for slot_name in ("current", "next_1", "next_2"):
                        ...
                        if slot_name in sim_orders and states[slot_name] in (STATE_WATCHING, STATE_ARMED) and new_state == STATE_EXIT_TRIGGERED:
                            if sim_orders[slot_name] is not None:
                                print(f"[SIM] {slot_name} sim expired due to exit trigger")
                                states[slot_name] = STATE_SIM_EXPIRED
                                new_state = STATE_SIM_EXPIRED

                        if slot_name in sim_orders and new_state in (STATE_WATCHING, STATE_ARMED) and sim_orders[slot_name] is None and metrics:
