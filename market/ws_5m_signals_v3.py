...
def _signal_label(metrics: Optional[Dict[str, float]], stable_count: int) -> str:
    ...
    if metrics["sum_asks"] <= ARBITRAGE_SUM_ASKS_MAX:
        return "arbitrage_candidate"
    if metrics["sum_asks"] <= WATCH_SUM_ASKS_MAX:
        return "near_arb_zone"
    return "normal"


def _print_slot(slot_name: str, snap: Dict[str, Any], secs_to_end: Optional[int], trigger: int, stable_count: int, last_label: Optional[str]) -> str:
    up = snap.get("up")
    down = snap.get("down")
    print(f"\n[{slot_name.upper()}]")
    print(f"secs_to_end={secs_to_end} | stable_count={stable_count}")
    if up:
    ...
    label = _signal_label(metrics, stable_count)
    if metrics:
    ...
    if label != last_label and slot_name in ("next_1", "next_2"):
        if label == "arbitrage_candidate":
            print(f"[ALERT] arbitrage_candidate entered")
        elif label == "near_arb_zone":
            print(f"[WATCH] near_arb_zone entered")
        elif label == "normal" and last_label in ("arbitrage_candidate", "near_arb_zone"):
            print(f"[INFO] signal zone left")
...
async def monitor_5m_signals_v3(duration_seconds: int = 20) -> None:
    ...
    last_labels = {"current": None, "next_1": None, "next_2": None}
            ...
            while time.time() - started_at < duration_seconds:
                ...
                if time.time() - last_print >= 2:
                    ...
                    for slot_name in ("current", "next_1", "next_2"):
                        ...
                        new_label = _print_slot(slot_name, snap, secs, trigger, stable_counts[slot_name], last_labels[slot_name])
                        last_labels[slot_name] = new_label
        finally:
            hb.cancel()
