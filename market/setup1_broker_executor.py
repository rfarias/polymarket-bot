...
from market.setup1_order_manager_v2 import Setup1OrderManagerV2, PLAN_DONE, PLAN_FORCE_CLOSED
...
class Setup1BrokerExecutor:
    ...
    def _place_or_shadow_order(self, *, plan_id: str, slot_name: str, event_slug: str, req: BrokerOrderRequest, leg: str) -> List[str]:
        if self.shadow_only:
    ...
    def evaluate_slot(self, *, slot_name: str, event_slug: str, signal: str, metrics: Optional[Dict[str, float]], secs_to_end: Optional[int]) -> List[str]:
        ...
        logs.extend(self._place_or_shadow_order(plan_id=created_plan.plan_id, slot_name=slot_name, event_slug=event_slug, req=up_req, leg="up_entry"))
        logs.extend(self._place_or_shadow_order(plan_id=created_plan.plan_id, slot_name=slot_name, event_slug=event_slug, req=down_req, leg="down_entry"))
        return logs
