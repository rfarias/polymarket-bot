...
def _outcome_for_entry(entry_leg: str) -> str:
    return "UP" if entry_leg.startswith("up") else "DOWN"
...
def _post_real_exit_order(executor, slot_name: str, plan_id: str, entry_leg: str, exit_price: float, reason: str) -> List[str]:
    ...
    req = BrokerOrderRequest(
        token_id=f"{plan.event_slug}:{_outcome_for_entry(entry_leg)}",
        side="SELL",
        price=float(exit_price),
        size=float(qty),
        market_slug=plan.event_slug,
        outcome=_outcome_for_entry(entry_leg),
...
def maybe_take_single_leg_profit_real_v1(executor, *, slot_name: str, metrics: Optional[Dict[str, float]], tick_size: float = DEFAULT_TICK_SIZE) -> List[str]:
...
def maybe_post_force_close_exits_v1(executor, *, slot_name: str, metrics: Optional[Dict[str, float]]) -> List[str]:
    plan = _plan(executor, slot_name)
    if plan is None or not metrics:
        return []
...
def maybe_post_balanced_exit_orders_v1(executor, *, slot_name: str, metrics: Optional[Dict[str, float]]) -> List[str]:
    plan = _plan(executor, slot_name)
    if plan is None or not metrics:
        return []
...
def handle_deadline_real_v1(executor, *, slot_name: str, secs_to_end: Optional[int], deadline_trigger: Optional[int], metrics: Optional[Dict[str, float]]) -> List[str]:
    ...
    if up.filled_qty == down.filled_qty and up.filled_qty > 0:
        ...
        logs.extend(maybe_post_balanced_exit_orders_v1(executor, slot_name=slot_name, metrics=metrics))
        return logs
    ...
    logs.extend(maybe_post_force_close_exits_v1(executor, slot_name=slot_name, metrics=metrics))
    return logs


def cleanup_terminal_plan_v1(executor, *, slot_name: str) -> List[str]:
    plan = _plan(executor, slot_name)
    if plan is None:
        return []
    logs: List[str] = []
