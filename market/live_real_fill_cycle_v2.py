...
def _validate_cfg(cfg) -> bool:
    if not cfg.enabled:
        ...
        return False
    if cfg.shadow_only:
        print("[GUARD] live_real_fill_cycle_v2 requires POLY_GUARDED_SHADOW_ONLY=false")
        return False
    if not cfg.real_posts_enabled:
        print("[GUARD] live_real_fill_cycle_v2 requires POLY_GUARDED_REAL_POSTS_ENABLED=true")
        return False
    if cfg.allow_next_2:
        print("[GUARD] live_real_fill_cycle_v2 requires POLY_GUARDED_ALLOW_NEXT_2=false")
        return False
    if cfg.max_active_plans != 1:
        print("[GUARD] live_real_fill_cycle_v2 requires POLY_GUARDED_MAX_ACTIVE_PLANS=1")
        return False
    if cfg.min_shares_per_leg != 5:
        print("[GUARD] live_real_fill_cycle_v2 expects POLY_GUARDED_MIN_SHARES=5")
        return False
    return True


def monitor_live_real_fill_cycle_v2(duration_seconds: int | None = None) -> None:
    cfg = load_live_guarded_config()
    print("[LIVE_GUARDED_CONFIG]", cfg.as_dict())
    if not _validate_cfg(cfg):
        return

    test_deadline_trigger_secs = _env_int("POLY_FILL_TEST_DEADLINE_TRIGGER_SECS", 120)
    min_post_buffer_secs = _env_int("POLY_FILL_TEST_MIN_POST_BUFFER_SECS", 30)
    ...
    print(
        "[FILL_CYCLE_CONFIG]",
        {
            ...
            "test_deadline_trigger_secs": test_deadline_trigger_secs,
    ...
    while time.time() - started_at < run_for:
        ...
        if time.time() >= next_print:
            ...
            print("\n===== LIVE REAL FILL CYCLE V2 SNAPSHOT =====")
        ...
        if plan is None and event_slug_next1 and tradable_signal_next1 == cfg.require_signal:
            ...
            if next_1_secs is not None and next_1_secs <= min_entry_secs:
                print(
                    f"[ENTRY_GATE] next_1 armed but secs_to_end={next_1_secs} <= min_entry_secs={min_entry_secs}; waiting for next cycle"
                )
            else:
        ...
        try:
            ...
            deadline_logs = handle_deadline_real_v2(
                ...
                slot_name="next_1",
                secs_to_end=next_1_secs,
                deadline_trigger=test_deadline_trigger_secs,
                metrics=tradable_metrics_next1,
            )
...
if __name__ == "__main__":
    monitor_live_real_fill_cycle_v2()
