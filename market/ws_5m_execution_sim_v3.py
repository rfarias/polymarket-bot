...
def _entry_quality_check(metrics: Dict[str, float], slot_name: str, secs_to_end: Optional[int]) -> tuple[bool, str]:
    reasons: list[str] = []
    ...
    if metrics["sum_bids"] < MIN_SUM_BIDS_TO_PLAN:
        reasons.append(f"sum_bids {metrics['sum_bids']} < {MIN_SUM_BIDS_TO_PLAN}")

    projected_exit_up = round(metrics["up_ask"] + DEFAULT_TICK_SIZE, 2)
    projected_exit_down = round(metrics["down_ask"] + DEFAULT_TICK_SIZE, 2)
    exit_gap_total = round((projected_exit_up - metrics["up_bid"]) + (projected_exit_down - metrics["down_bid"]), 4)
...
async def simulate_5m_execution_v3(duration_seconds: int = 40) -> None:
                ...
                if time.time() - last_print >= 2:
                    ...
                    for slot_name in ("current", "next_1", "next_2"):
                        ...
                        if slot_name == "next_1" and new_state == STATE_EXIT_TRIGGERED and sim_orders[slot_name] is None and not focus_announced:
                            ...
                            ok, reason = _entry_quality_check(metrics, slot_name, secs)
                            if ok:
                        ...
                        print(f"\n[{slot_name.upper()}]{marker} secs_to_end={secs} | stable_count={stable_counts[slot_name]} | state={states[slot_name]}")
                        if metrics:
