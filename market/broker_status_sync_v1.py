...
def sync_executor_from_broker_open_orders(executor, broker_open_orders: List[Any]) -> Tuple[List[str], Dict[str, Any]]:
    logs: List[str] = []
    reconcile = reconcile_executor_with_broker_open_orders(executor, broker_open_orders)

    tracked_rows = reconcile.get("tracked") or []
    for row in tracked_rows:
        ...
        broker_filled = int(round(float(row.get("remaining_size", 0.0) + float(getattr(ticket, 'filled_qty', 0)))))
        # Prefer deriving from stored broker order snapshot when available.
        payload = (executor.plan_broker_orders.get(plan_id) or {}).get(leg) or {}
        payload_order = (payload.get("order") or {})
        if payload_order:
            try:
                broker_filled = int(round(float(payload_order.get("size_matched") or ticket.filled_qty or 0)))
            except Exception:
                broker_filled = int(ticket.filled_qty)
        # Reconciler already refreshed payload order when possible; fall back to row if needed.
        try:
            row_status = str(row.get("status") or payload_order.get("status") or "").lower()
        except Exception:
            row_status = ""
        ...
        if fill_delta > 0:
            ...
            logs.append(f"[BROKER_SYNC] {slot_name}: {leg} fill_delta={fill_delta} status={row_status}")
    ...
    # Close runtimes whose plans became terminal after sync.
