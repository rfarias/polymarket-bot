...
from market.current_scalp_signal_v1 import (
    CurrentScalpMicroConfigV1,
    CurrentScalpMicroResearchV1,
...
def _build_default_log_path() -> Path:
    ...
    return Path("logs") / f"current_scalp_micro_paper_{ts}.jsonl"
...
def _paper_enter(signal: dict, tick_size: float, now: float, cfg: CurrentScalpMicroConfigV1) -> PaperTrade:
    trade = PaperTrade()
    trade.mode = "open"
...
def _paper_manage(
    ...
    *,
    ...
    elif not signal.get("allow") and str(signal.get("reason") or "").startswith("no_micro_refill_edge"):
        trade.mode = "idle"
        trade.exit_price = bid_now if bid_now > 0 else trade.entry_price
        trade.exit_reason = "refill_faded"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Paper-trade current scalp micro imbalance setup")
    ...
    cfg = CurrentScalpMicroConfigV1()
    research = CurrentScalpMicroResearchV1(cfg=cfg)
    trade = PaperTrade()
    ...
    print("[CURRENT_SCALP_MICRO_CONFIG]")
    pprint(cfg.as_dict())
    print("[LOG_FILE]")
    print(log_path)
    ...
    while time.time() - started_at < args.seconds:
        ...
        if not current_item:
            print("[CURRENT_SCALP_MICRO] current slot unavailable")
    ...
    print("[CURRENT_SCALP_MICRO_SUMMARY]")
    pprint(summary)
    return 0
