...
from market.current_scalp_signal_v1 import (
    CurrentScalpCenterConfigV1,
    CurrentScalpCenterResearchV1,
...
def _build_default_log_path() -> Path:
    ...
    return Path("logs") / f"current_scalp_center_paper_{ts}.jsonl"
...
def _paper_enter(signal: dict, tick_size: float, now: float, cfg: CurrentScalpCenterConfigV1) -> PaperTrade:
    trade = PaperTrade()
    trade.mode = "open"
...
def _paper_manage(
    ...
    *,
    ...
    signal: dict,
    cfg: CurrentScalpCenterConfigV1,
) -> PaperTrade:
    if trade.mode != "open":
        return trade
    ...
    elif bool(signal.get("in_center_band")):
        trade.mode = "idle"
        trade.exit_price = bid_now if bid_now > 0 else trade.entry_price
        trade.exit_reason = "returned_to_center"
    ...
    elif signal.get("regime") != "center_lateral":
        trade.mode = "idle"
        trade.exit_price = bid_now if bid_now > 0 else trade.entry_price
        trade.exit_reason = "regime_break"
    elif not signal.get("allow") and str(signal.get("reason") or "") in {
        "below_center_without_reclaim",
        "above_center_without_fade",
        "no_center_edge",
    }:
        trade.mode = "idle"
        trade.exit_price = bid_now if bid_now > 0 else trade.entry_price
        trade.exit_reason = "center_reversion_faded"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Paper-trade current scalp center-reversion setup")
    ...
    cfg = CurrentScalpCenterConfigV1()
    research = CurrentScalpCenterResearchV1(cfg=cfg)
    trade = PaperTrade()
    ...
    print("[CURRENT_SCALP_CENTER_CONFIG]")
    pprint(cfg.as_dict())
    print("[LOG_FILE]")
    print(log_path)
    ...
    while time.time() - started_at < args.seconds:
        ...
        if not current_item:
            print("[CURRENT_SCALP_CENTER] current slot unavailable")
        ...
        if trade.mode == "idle" and signal.get("allow"):
            ...
            trade = _paper_manage(trade, bid_now=bid_now, tick_size=tick_size, now=now, secs_to_end=secs_to_end, signal=signal, cfg=cfg)
            if trade.mode == "idle":
    ...
    print("[CURRENT_SCALP_CENTER_SUMMARY]")
    pprint(summary)
    return 0
